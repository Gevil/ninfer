...
**V2-T8 runtime gate r7 (2026-09-10, RUNNING):** r7 (`b7ff49a7`) (1) charges the paged-KV image bytes into `KVRamCache`'s total host footprint — `capture()` now enforces `capacity_bytes` over the TOTAL (evicting unpinned records until the prospective record fits; when it cannot, a graceful journal drop `capture drop reason=host-kv-arena-full` instead of a settle-path throw — a full shared host KV arena no longer takes the settle hook down); (2) adds `[t8]` journal instrumentation (capture ok / evict / budget drop / match hit / match miss / restore) + `kv_image_bytes` in the snapshot; (3) the window tests the **designed continuation flow** — s2 = [u(s1), a(s1-completion), u(new question)] must hit via `AppendAtFrontier` (thinking off so the assistant turn round-trips the template exactly), s3 = same shape with a different 16k prefix (negative control). Window `v2t8ab-r7-2026-09-10` under supervisor (shipwatch) + watchdog + always-restore; phase C runs `--kv-ram-capacity-mib 1024` (1 GiB tier budget — ~8 records of 129 MiB fit, so the probe sequence must stay inside via eviction). Verdict pending.
