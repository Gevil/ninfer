...
- **V2-T2 — CONFLICT (smallest manual surface):** picks 15, 16, 17 (`6a1b62c5`, `27417ca2`, `adf494c2`) — no missing substrate; conflicts confined to 7 existing serve/device files (§6.1), per-hunk resolution against the upstream serve/device refactor. Chain measured cumulatively (each pick against the preceding picks' state): the conflict set persists across the chain, and pick 17's surface grows to 4 files in-chain.
...
| **V2-T2** | gpillon agentic picks: `6a1b62c5` (warmup/deadline decouple), `27417ca2` (warmup fail-fast + auto kv-capacity bounds), `adf494c2` (block host sync — the 100 %-CPU decode fix) — per-hunk conflicts on 7 existing serve/device files, no substrate; conflict set persists under cumulative chain application | `gpillon/gpillon/coding` (conflict surface measured, §6.1) | per-file hunk review vs the upstream serve/device refactor; ctest; idle-CPU check during decode (the 100 %-CPU regression) + TTFT |
