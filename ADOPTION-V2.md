...
| **V2-T11 — SHIPPED (2026-09-11, A/B-gated; first red was a drift artifact)** | The 12 clean picks of `md/*` re-applied on `v2/t10-upstream-sync` (branch `v2/t11-md-clean` @ 723d22d1; image `v2t11-723d22d1` / 5cf21a94, now :quasar/:latest). First ship: battery 12/16 red on DECODE gates (151.8/152.1 vs floors 154.9/154.0 from the high-clock T10 baseline) → auto-rollback; same-window A/B #1 read −5.6% fresh → initially REJECTED. **Bisection (all clean same-window, session silent, supervised):** W8 small-T `b081714c` EXONERATED (0.993/1.001), FP16-PV `e84d7ea9` exonerated (≥ clean T10), then group split — GROUP-ATT (#225 trio + #222 pair: 1.101x fresh / 0.999x 8k) and GROUP-COST (GDN norm + TMA scale + #195 trio: 1.054x / 1.011x): **no subset regressed** (window t11grp, in-window builds, lane stopped per 09-11 rule). **Full-wave recheck A/B (window t11ab2, ShipwatchT11AB2, settled box, clocks 2902/2917 MHz, zero stalls): PASS — CAND 148.6 fresh / 139.8 8k vs LIVE 143.4 / 140.0 = 1.036x / 0.999x (≥0.97x both legs) → T11 SHIPPED** (PASS leaves candidate live; quadlet byte-identical, /v1/models 200). Root cause of the initial red: **intraday box drift** (same-day T10 LIVE readings 165.1 → 151.0 → 143.4 against a fixed morning baseline) — not a code regression. Battery baseline re-pinned to `quasar-baseline-2026-09-11-t11.json` (148.6/139.8). **Drift-balanced confirmation (window t11cfm, ShipwatchT11CFM, bracketed T11→T10→T11, session silent, SM 2902→2917 MHz):** T11 legs 154.1/151.3 + 152.8/143.9 vs T10 157.8/149.5 (fresh/8k) → fresh 97.2–97.7%, 8k 98.7–101.2% — PASS under both the max and mean 2-leg conventions (≥97% gate; the window script's 2-leg "median" is the max — both conventions pass) → T11 CONFIRMED. The −5.6% was never reproduced in any of the four windows — t11ab-specific confound (session load + box-state swing between legs); net verdict: T10 vs T11 decode-parity within ±5% window noise. The wave's value is prefill-side (#195 context-cost fallback 3.1x→1.15x), which the decode probes don't measure — no-regression + prefill gain is the expected and confirmed outcome | `md/*` (the 7 picks, re-applied as the 12-clean-pick T11 branch) | done: build + battery + 4 A/B windows + group bisection + drift-balanced confirmation (supervisors ShipwatchT11AB/Grp/AB2/CFM) |
