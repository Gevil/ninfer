...
| **V2-T10** | Upstream master sync: `9f0575bb` (q4 topk bf16 build fix) + `d4929686` (materialization search + adaptive planning budgets: `materialization_planner.h` 729-line rework, new `materialization_budget.h`, request diagnostics, 7 CTests; targets the fixed-5 ms pressure-search budget that new issue #229 reports). Merge probes: CLEAN into `quasar-master` AND into `v2/quasar-gpillion` (7 files both sides touch auto-merge). No `dflash*` files touched (§11.4) | `upstream/master` | build + ctest + battery + multi-session replay + decode A/B (ship window, supervisor) |
| **V2-T11** | md clean-pick wave — all merge-tree CLEAN + `cherry +` vs `v2/quasar-gpillion`: `ffb0be45` (attention: accumulate PV in FP16 inside each key block; 4f +96/−28) · `974d36e8` (GDN: spread fused input normalization across row-tile-zero warps; 1f +18/−12) · `c0fbb974` (W8 small-T: keep two k-groups in flight — our C=4 small-T regime; 3f +99/−34) · `d63848ec` (#225: sigmoid gate folded into causal-attention reduce epilogue + FP64 oracle; 3 commits) · `ac6a309c` (#222: `rmsnorm_rope` extended to the text profile — one node instead of three) · `8881952f` (#160: nvfp4 TMA tile-contiguous activation scales; prefill +4.1 % "bitwise identical"; 11f +100/−34) · `f0391cad` (#195: context-cost weights-format fallback; prefill-cost 3.1× → 1.15×). All sit on `b88c0f6f` (our merge base) → re-probe after V2-T10. Remainder, CONFLICT → hand-port: `dc108d43` (#194: keep approximate SiLU off the `__fdividef` zeroing range — new on top of our shipped `61250e89`) · `d621ba1e` (#201: predicated W8 cache policy — verified NOT a descendant of our T5 pick `ed150906`) | `md/prep/*`, md PR heads | build + ctest + battery + decode A/B |
...
## 11. Re-audit 2026-09-11 (fetch + read-only probes; no lane contact)
