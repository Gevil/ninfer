...
| **V2-T3 — DEFERRED (measured 2026-09-09, 3 ship attempts)** | gpillon PATH-REMAP pick `5f014910` (tool-call XML leak: the base's `ToolCallOutputDecoder::finish()` restructured onto the restructured decoder — the tool-call protocol region is no longer replayed as visible content; the fallback test updated to the no-leak behavior). `093c1fdd` (sibling-overlap telemetry) deferred — pure measurement + needs the dropped `concurrent_executor.h` re-targeted. **MEASURED (3 supervised ship attempts, branch `v2/t3-path-remap` @ `458376c4`, image `12b87f4d`):** built clean + ctest clean (incl. the updated `test_tool_call_parser`) + 15/16 battery (every functional check passed: vision x3, replay 4/4, think-smoke, xhigh, quality, soak 5/5, 4xx-watch). DECODE-FRESH PASSED all 3 (163.9 / 159.2 / 171.4 tps); DECODE-8K FAILED all 3 (142.7 / 144.9 / 131.6 tps). **Root cause: NOT a V2-T3 regression** — the live V2-T2 baseline image (`408c7df8`) itself measures ~137–160 tps on the same 8k probe (median 137.6), so the V2-T3 (131–145) is **decode-neutral**. The 8k decode is in an intraday **power-constrained drift regime**: GPU at 544/575 W (95 % power limit), 2887/3090 MHz (93 % SM clock), 99 % util — the stored 8k baseline (165.3, re-baselined 153.8) is a stale high-water mark, and the DECODE-8K gate (≥95 % of a drifting baseline) is unreliable in this state (the 8k probe is more power-sensitive than the fresh probe). **Verdict: DEFERRED** — the fix is decode-neutral + the gate is red only due to the power-limited GPU state; re-attempt the ship when the GPU is in a stable (non-power-limited) state, or relax the DECODE-8K gate for power-constrained windows. Branch + image retained for re-ship. | `gpillon/gpillon/coding` | tool-call parser unit tests; telemetry = no behavior change |
