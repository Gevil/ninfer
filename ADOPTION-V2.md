...
| **V2-T11 — REJECTED (2026-09-11, measured), bisection running** | battery 12/16 red on DECODE gates (CAND medians fresh 151.8 / 8k 152.1 vs floors 154.9/154.0 = T10 baseline −6%) → auto-rollback to T10 `ce6c719f`. Same-window A/B (supervised, T10 pattern) **confirmed a real decode regression, not an artifact**: LIVE T10 medians 165.1 fresh / 147.2 8k vs CAND 155.9 / 144.1 (−5.6% fresh / −2.1% 8k); all other gates green. **Round-1 bisection (clean same-window rechecks): W8 small-T `b081714c` EXONERATED (ratio 0.993 fresh / 1.001 8k); FP16-PV `e84d7ea9` exonerated (at/above clean T10)** — note: the first bisection window's legs were confounded by session traffic + box reclaim (LIVE leg had a 30.8s stall); clean rechecks require the OMP session silent during measure phases. **Round-2 running (2026-09-11): group test** — GROUP-ATT `v2/t11grp-att` @ 55009f3d (#225 smigmoid trio + #222 rmsnorm pair) vs GROUP-COST `v2/t11grp-cost` @ 25462dcf (GDN-norm + TMA-scale #160 + #195 trio), each 5 picks on T10; window `t11grp-window-2026-09-11.sh` (LIVE → ATT → COST → always-restore-LIVE; both images built in-window with the lane stopped; supervisor `ShipwatchT11Grp`). Next: offending group → per-pick bisection inside it; culprit(s) isolated → re-propose wave minus them | `md/*` (the 7 picks, re-applied as the 12-clean-pick T11 branch `v2/t11-md-clean` @ 723d22d1) | decode A/B vs T10 baseline — FAILED; group bisection round 2 in progress |
