...
**V2-T8 runtime gate r7 (2026-09-10):** r7 (`b7ff49a7`) (1) charges the paged-KV image bytes into `KVRamCache`'s total host footprint — `capture()` now enforces `capacity_bytes` over the TOTAL (evicting unpinned records until the prospective record fits; when it cannot, a graceful journal drop `capture drop reason=host-kv-arena-full` instead of a settle-path throw — a full shared host KV arena no longer takes the settle hook down); (2) adds `[t8]` journal instrumentation (capture ok / evict / budget drop / match hit / match miss / restore) + `kv_image_bytes` in the snapshot; (3) the window tests the **designed continuation flow** — s2 = [u(s1), a(s1-completion), u(new question)] must hit via `AppendAtFrontier` (thinking off so the assistant turn round-trips the template exactly), s3 = same shape with a different 16k prefix (negative control). First window run aborted at the build SHA precheck (branch push had gone to the wrong remote — the window's clone syncs from the GitHub fork; fixed: `v2/t8-agentic` re-pointed + pushed to the fork, ls-remote-verified). Second run passed gate (115/115) + build (candidate `fd2f5bfa`) but crash-looped at phaseA on a **transient GPU condition**: a leaked context from the gate's GPU tests held ~8.8 GiB (total 31.2/32.6 GiB in use) so the engine's 8.14 GiB runtime reservation found only 1.33 GiB free — the window's + watchdog's restore paths then stalled on systemd's start-limit; supervisor force-restore recovered the lane in 14 s (verified: quadlet ≡ BAK, image = live, template sha intact). Pipeline fixed: `gpu_orphan_sweep` + `wait_vram_clear` after the gate and before each phase start; window `restore()` is now a 5-attempt loop (stop + `reset-failed` + sweep + VRAM wait); watchdog `force_restore` retries 3x. **Run 3 (`v2t8ab-r7b-2026-09-10`) launched** under supervisor + watchdog + always-restore; phase C runs `--kv-ram-capacity-mib 1024` (1 GiB tier budget — ~8 records of 129 MiB fit, so the probe sequence must stay inside via eviction). Verdict pending.
