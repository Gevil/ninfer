...
| **V2-T11 — LAUNCHED (2026-09-11 14:05, pipeline running; branch `v2/t11-md-clean` @ 723d22d1)** | md clean-pick wave — cherry-picked cleanly in sequence onto the post-T10 tree: **12 commits** (the audit's standalone `apply --3way` probes over-flagged — cherry's 3-way merge resolved what they rejected; and the audit's pick list had missed #222's parent `5f7a4c4a` + #195's series root `0f84adaf`+`5ae1335b` — all now included): FP16-PV accumulation (`ffb0be45`) · GDN norm warp (`974d36e8`) · W8 small-T 2 k-groups (`c0fbb974`) · #225 trio (`9fe228a3`/`2b7fb9e7`/`d63848ec`) · #222 pair (`5f7a4c4a`+`ac6a309c` — text-form extension + ceiling fix; the parent was the missing substrate) · #160 TMA tile-contiguous scales (`8881952f`) · #195 trio (`0f84adaf`/`5ae1335b`/`f0391cad` — fallback + tests + reporting) | md prep/PR heads | ship pipeline (supervisor ShipwatchT11): build + ctest + battery vs the refreshed T10 decode baseline |
