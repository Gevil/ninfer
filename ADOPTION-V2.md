...
| Image | Baseline image `localhost/ninfer-nvfp4:qm-f7727926`. **As of 2026-09-09, V2-T1 (PR #211) has shipped on top** — the live image is the V2-T1 build from branch `v2/t1-stream-kv` @ `ba21e676`; the quadlet's `Image=` is `:quasar` (retagged to that build, also tagged `:v2t1-ba21e676`); quadlet `ninfer-nvfp4`, port 8002→8080, 2 Volume mounts |
...
- **V2-T2 — RE-DERIVED 2026-09-09: pick 17 only.** The §6.1 dry-run measured the conflict *surface* (picks 15–17 3-way-conflict on 7 existing serve/device files). Re-deriving the *content* against the real base: picks 15 (`6a1b62c5`, warmup/deadline decouple) and 16 (`27417ca2`, warmup fail-fast + auto kv-capacity) are **SUPERSEDED by the base** — warmup already uses `DeadlinePolicy::UnboundedStartup` (deadline decoupled; the 60s cap intentionally not adopted) and `apps/serve/main.cpp` already fails fast (`return 1`) on warmup failure (residual = cosmetic usage-text strings). So V2-T2 = **pick 17 (`adf494c2`, the 100 %-CPU blocking-sync fix) only**, adapted to the 2-stream base (gpillon's single-stream `load_stream` → our `transfer_stream`; CUDA 13.1.2 3-arg `cudaInitDevice`). **SHIP ATTEMPTED 2026-09-09, ROLLED BACK** (supervised pipeline, branch `v2/t2-agentic` @ `0faef6d4`, image `408c7df8`): built + ctest PASS + booted, but battery **15/16** — the sole red is the cold **decode-fresh** probe (145.4 vs the 153.9 gate; decode-8k 157.3 PASSed its 157.0 gate). Both candidate probes landed ~5–10 % below the V2-T1 baseline (fresh 162.0 / 8k 165.3), so this reads as a small **per-round latency from the blocking sync** — a real but modest trade-off against the host-core savings the fix buys — not single-sample noise. **Re-probe (clean repeated fresh+8k A/B vs the V2-T1 baseline) before re-attempting.**
...
**Status as of 2026-09-09:** **V2-T1 has shipped** (branch `v2/t1-stream-kv` @ `ba21e676`, image tag `v2t1-ba21e676`); **V2-T2 SHIP ATTEMPTED 2026-09-09, ROLLED BACK** (branch `v2/t2-agentic` @ `0faef6d4`, tag `v2t2-0faef6d4`; battery 15/16 — cold decode-fresh FAIL 145.4 vs 153.9, decode-8k PASS 157.3; both probes ~5–10 % below the V2-T1 baseline → a real modest decode trade-off for the host-core savings; re-probe before re-attempting; pick 17 only, §6.2); V2-T3…V2-T9 remain pending.
...
| **V2-T2** | **pick 17 `adf494c2` (block host sync — the 100 %-CPU decode fix) only.** Re-derived 2026-09-09: picks 15 (`6a1b62c5`) & 16 (`27417ca2`) are **SUPERSEDED by the base** (warmup already `DeadlinePolicy::UnboundedStartup` + fail-fast `main.cpp`; residual = cosmetic strings) — §6.2. Adopted = `src/core/device.{cu,h}` (2-stream adaptation; CUDA 13.1.2 3-arg `cudaInitDevice`). **SHIP ATTEMPTED 2026-09-09, ROLLED BACK** (branch `v2/t2-agentic` @ `0faef6d4`, tag `v2t2-0faef6d4`; image `408c7df8` retained-unserved): built + ctest PASS + booted, battery 15/16 (cold decode-fresh FAIL 145.4 vs 153.9, decode-8k PASS 157.3) — a real ~5–10 % decode trade-off for the host-core savings; re-probe (clean fresh+8k A/B vs V2-T1) before re-attempting. | `gpillon/gpillon/coding` | ctest (failure-set diff); battery 16/16; idle-host-CPU check during decode (the 100 %-CPU regression) |
| **V2-T3** | gpillon PATH-REMAP picks: `5f014910` (tool-call XML leak: remap `src/serve/tool_call_parser.cpp` → `src/targets/qwen3_6/impl/frontend/tool_call_parser.cpp`; the test 3-way-merges), `093c1fdd` (sibling-overlap telemetry: drop the `concurrent_executor.h` hunk) | `gpillon/gpillon/coding` | tool-call parser unit tests; telemetry = no behavior change |
| **V2-T4** | re-adopt our own still-unique shipped work: `52fabe3e` + `bb535075` (T23 TMA pair — #167/#160 still open upstream), `49400365` (T18 GDN chunked-prefill accuracy), `15729d9b` + `2e99db7b` (T17 FP16-PV re-picks), `67bf4b78` (T36 softmax-fold re-pick), `0f84adaf` (T43/#195 context-cost fallback), `ed505ebc` + `d3278b79` (t42wave fusions), `fa12e8ef` (t42wave MTP stem-norm/tail-residual fusion — content = md's `1f155fed`; MTP-conditional) | our branches (`t23-tma-pair`, `t18-gdn-quasar`, t15-yarn line, `t36-mdops-quasar`, `t42wave-quasar`) | per-pick `git merge-tree` clean; ctest; decode A/B vs the 189.5/140.4/51.2 tps anchors |
