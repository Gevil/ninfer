...
| **V2-T11 — IN TRIAGE (2026-09-11: ship FAIL on DECODE gates only; A/B window running; branch `v2/t11-md-clean` @ 723d22d1)** | md clean-pick wave — cherry-picked cleanly in sequence onto the post-T10 tree: **12 commits** (the audit's standalone `apply --3way` probes over-flagged — cherry's 3-way merge resolved what they rejected; and the audit's pick list had missed #222's parent `5f7a4c4a` + #195's series root `0f84adaf`+`5ae1335b` — all now included): FP16-PV accumulation (`ffb0be45`) · GDN norm warp (`974d36e8`) · W8 small-T 2 k-groups (`c0fbb974`) · #225 trio (`9fe228a3`/`2b7fb9e7`/`d63848ec`) · #222 pair (`5f7a4c4a`+`ac6a309c`) · #160 TMA tile-contiguous scales (`8881952f`) · #195 trio (`0f84adaf`/`5ae1335b`/`f0391cad`). **Ship (14:05-14:30, supervisor ShipwatchT11):** build ~9 min (warm cache), ctest PASS (skips within baseline), battery 14/16 — DECODE-FRESH 151.8 (floor 154.9) + DECODE-8K 152.1 (floor 154.0) red, everything else green (vision 4-4, replay, think-smoke, xhigh, quality 5/5, soak, 4xx) → auto-rollback to T10 (verified). T10-pattern triage: same-window A/B (CAND 5cf21a94 vs LIVE ce6c719f, best-of-3 medians, ≥97 % gate) deciding real regression vs single-cold-shot artifact | md prep/PR heads | same-window A/B (supervisor ShipwatchT11AB) → ship or reject |
