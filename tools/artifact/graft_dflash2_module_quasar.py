...
def main() -> None:
    ...
    fmax = local.get("relative_frobenius_error_max")
    fmean = local.get("relative_frobenius_error_mean")
    if fmax is None:
        print("module relative Frobenius error: n/a (BF16 direct copy, no re-quantization)")
    else:
        print(f"module relative Frobenius error: max {fmax:.6f} mean {fmean:.6f}")
