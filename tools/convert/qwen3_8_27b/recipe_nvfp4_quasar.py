...
QUANTIZED_REVISION = "d5aebc7ca162069be2331603528b735e4e0dcceb"
...
# The two vocabulary endpoints are kept in full BF16 and sourced bit-exact from the
# QUASAR checkpoint (its post-QAT token_embedding and lm_head) rather than from the
# official BF16 checkpoint used for the other non-backbone components.
QUASAR_ENDPOINT_RECIPES = (
    ...
    family_recipe.TensorRecipe(
        "text/output_head", family_recipe.source("lm_head.weight", (248320, 5120))
    ),
)

_QUASAR_DIRECT_RECIPES_BY_NAME = {
    item.object_name: item
    for item in shared.QUANTIZED_DIRECT_RECIPES
    if item.object_name not in CONTROLS_BY_NAME
}
_QUASAR_DIRECT_RECIPES_BY_NAME.update(
    {item.object_name: item for item in QUASAR_ENDPOINT_RECIPES}
)
QUANTIZED_DIRECT_RECIPES = tuple(
    _QUASAR_DIRECT_RECIPES_BY_NAME[spec.name]
    for spec in inventory.TEXT_CORE_TENSOR_SPECS
    if spec.name in _QUASAR_DIRECT_RECIPES_BY_NAME
)
...
OFFICIAL_TENSOR_SPECS = tuple(
    ...
    if spec.name.startswith("text/draft_head")
...
def validate_recipe() -> None:
    ...
    ) != (256, 256, 128, 400, 48, 96, 496, 355, 347):
