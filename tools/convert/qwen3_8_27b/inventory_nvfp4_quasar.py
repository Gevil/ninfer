...
# Distinct identity from the mixed FP8/NVFP4 "nvfp4" artifact: the runtime
# resolves this one to the QUASAR backbone profile with BF16 vocabulary
# endpoints (WeightsProfile::Qwen38Nvfp4QuasarBf16).
WEIGHTS_ID = "nvfp4-quasar-bf16"
...
def _build_text_core_specs() -> tuple[TensorSpec, ...]:
    ...
    for original in base.TEXT_CORE_TENSOR_SPECS:
        ...
        if original.name in ("text/token_embedding", "text/output_head"):
            # The two vocabulary endpoints are preserved in full BF16 (bit-exact copy
            # of the QUASAR checkpoint tensors); the base profile would store them FP8.
            specs.append(tensor_spec(original.name, original.shape, BF16))
            continue
