...
// The prefix K/V inputs are always BF16. The K cache plane is always BF16; the V cache plane is
// either BF16 (bit-for-bit copy) or FP16 (the full-append BF16-cache convention: BF16 input
// converted to FP16 bits), selected per call site from the V plane dtype.
template <typename VCache>
struct KVCacheAppendPrefixValueStore {
    static __device__ __forceinline__ int4 from_bf16(const int4& bits) { return bits; }
};

template <>
struct KVCacheAppendPrefixValueStore<__half> {
    static __device__ __forceinline__ int4 from_bf16(const int4& bits) {
        return bf16x8_bits_to_f16x8_bits(bits);
    }
};
...
__device__ __forceinline__ void kv_cache_append_prefix_copy_cyclic_unit(
    ...
    __nv_bfloat16* __restrict__ cache_k, VCache* __restrict__ cache_v, int token, int unit_in_token,
    ...
    const int4 v0 = KVCacheAppendPrefixValueStore<VCache>::from_bf16(
    ...
    *reinterpret_cast<int4*>(&cache_v[dst]) = v0;
    ...
    const int4 v1 = KVCacheAppendPrefixValueStore<VCache>::from_bf16(
        *reinterpret_cast<const int4*>(&v[src + 8]));
...
__global__ void kv_cache_append_prefix_cyclic_kernel(
    ...
    VCache* __restrict__ cache_v, int min_count, int max_count, int width, int padded_capacity,
    int window) {
    ...
    kv_cache_append_prefix_copy_cyclic_unit<VCache>(k, v, cache_k, cache_v, token, unit_in_token,
                                                    slot, padded_capacity);
}
...
__global__ void kv_cache_append_prefix_paged_kernel(
    ...
    VCache* __restrict__ cache_v, const std::int32_t* __restrict__ block_tables, int physical_pages,
