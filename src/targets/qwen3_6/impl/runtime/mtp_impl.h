...
auto mtp_decode_batch_body(MtpBatchContext& state, std::int32_t batch_size, std::uint32_t k,
    ...
    return [&state, batch_size, k, envelopes] {
        ...
        // В приёмку уходят несрезанные тензоры: шаг строки берётся из их первого измерения,
        // а активных строк может быть меньше максимума конкурентности.
        ...
        // Срез под запись: сначала шаг, потом префикс активных строк — так он непрерывен.
        ...
        CUDA_CHECK(cudaMemcpyAsync(rec0.data, draft0.data, rec0.bytes(), cudaMemcpyDeviceToDevice,
                                   state.execution.device.stream));
        for (std::uint32_t step = 0; step + 1 < k; ++step) {
            ...
            Tensor next_prob = step_rows(draft_probs, static_cast<std::int32_t>(step + 1),
                                        batch_size);
            ...
            card.mtp_propose_batch(next_hidden, proposal_logits, next, &next_prob, &frontiers, s1,
                                   frame.sampling, &next_sup, &next_supp, &next_supn);
