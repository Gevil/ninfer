...
void complete_round_state_layout(LayoutBuilder& builder, RoundStateLayout& layout) {
    ...
    if (layout.spec.backend == SpeculativeBackend::Mtp) {
        ...
        decode.draft_support_ids = add_tensor(builder, DType::I32, {20 * batch, drafts},
                                              "MTP decode draft support ids");
        decode.draft_support_probs = add_tensor(builder, DType::FP32, {20 * batch, drafts},
                                                "MTP decode draft support probabilities");
        decode.draft_support_n = add_tensor(builder, DType::I32, {batch, drafts},
                                            "MTP decode draft support sizes");
        // Токен, для которого записана q: приёмка сверяет его с проверяемым черновиком.
        decode.draft_recorded_tokens = add_tensor(builder, DType::I32, {batch, drafts},
                                                  "MTP decode recorded draft tokens");
